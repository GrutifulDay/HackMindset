...
export function validateApiKey(expectedKey, routeDescription) {
  ...
  return async function (req, res, next) {
    ...
    const extensionHeader = rawAuthHeader.startsWith("Bearer ") ? rawAuthHeader.split(" ")[1] : "";

    // preklad aliasu na skutecny klic
    const realExtensionHeader =
      extensionHeader === "HACK_EXTENSION" ? HACK_EXTENSION : extensionHeader;
    ...
    const isFromAllowedSource =
      origin.includes(extensionID) || referer.includes(extensionID) || isLikelyFromChrome;
    ...
    return res.status(403).json({ error: "Neplatny API klic nebo neautorizovany zdroj" });
  };
}
